...
### Added
- Viewport renderer module (`debug-viewport-renderer.ts`) with GL state-restore unit tests.
- `requiresViewportRuntime()` exported from viewport planning helpers.
- Playwright headless flow specs and demo `?e2e=*` URL presets.

### Changed
- Headless runtime split: compose pipeline vs multi-pane viewport orchestration. Root API unchanged.
- Viewport renderer caches pane rects, documents compositor vs scissor differences, and dev-warns on graph mismatches.
...
### Changed
- `/r3f` is a thin adapter over the canonical core runtime (`debug-pipeline-runtime.ts`, `debug-views-r3f.tsx`, `debug-views-overlays.tsx`).
- Removed internal `debug-views-post.tsx`; public R3F API unchanged.

...
Use this mental model when configuring panes: `layout` decides the arrangement, `paneCount` decides how many panes are visible, and `viewportViews` decides what each explicit viewport pane renders. Providing `viewportViews` automatically uses the viewport/scissor renderer path.

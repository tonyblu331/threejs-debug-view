...
| --- | --- |
| `threejs-debug-view` | Canonical runtime: view definitions, render planning, TSL compositor, headless WebGPU helpers, and public types. |
| `threejs-debug-view/r3f` | Thin R3F adapter: `DebugViewLayer`, `DebugViews`, bundled Leva shell, and `useDebugViewsControls()`. |

See [Batteries Included](/threejs-debug-view/guides/batteries-included/) for what ships on npm versus the repo demo and docs site.

## Headless runtime

Import from `threejs-debug-view` when your app owns the WebGPU render loop. Full walkthrough: [Headless Runtime](/threejs-debug-view/guides/headless-runtime/).
...
The root export includes layout/pane helpers and the `DebugViewsOptions` type for custom UI:
...
Batteries-included overlay with default views and Leva controls.
...
Bundled Leva panel for debug overlay controls. Mount it yourself when the panel should live in a specific place in your app shell while sharing the same Leva store as `useDebugViewsControls`.
...
Returns Leva-backed control state to spread onto `DebugViews`. See [Quick Start — bring your own controls](/threejs-debug-view/guides/quick-start/#bring-your-own-controls-r3f).
...
| `lightComplexity` | Estimated light overlap heatmap (point/spot/rect lights; globals excluded). |

...
| `showLeva` | Mounts the bundled Leva control panel on `document.body`. Defaults to `true`. Set to `false` when mounting `DebugViewLeva` yourself or for capture-only flows. |
...
| `shaderCost` | Shader-complexity heatmap from shader-unit buckets. Click the viewport in the bundled overlay to sample a pixel; the legend shows **shader complexity** when idle. |
...
- `debug-pipeline-runtime.ts` — single-viewport `RenderPipeline` factory.
- `debug-viewport-renderer.ts` — multi-pane scissor/viewport orchestration (restores GL state after each frame).
- `debug-viewport-plan.ts` — pane assignments, cells, and `requiresViewportRuntime()`.
...
Pane rects from `createDebugViewportRects()` are cached when the renderer is constructed. Rebuild the renderer when the plan changes.

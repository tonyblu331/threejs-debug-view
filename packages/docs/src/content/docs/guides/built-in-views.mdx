...
Examples on this page use the R3F adapter; the same view list works in a headless loop — see [Headless Runtime](/threejs-debug-view/guides/headless-runtime/).
...
| `lightComplexity` | `heatmap` | Estimated dynamic light overlap on the default forward renderer. Shadows excluded in v1. | Renders only when selected or visible in a layout. |

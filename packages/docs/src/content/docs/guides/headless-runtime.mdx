...
The root export **`threejs-debug-view`** is the canonical runtime. The **`/r3f`** entrypoint is a thin adapter over the same pipeline.

Use this guide when:

- your app is not React-based
- you already own the render loop and UI
- you want full control over layout, overlays, and viewport presentation

For install commands, see [Quick Start — without R3F](/threejs-debug-view/guides/quick-start/#install-without-r3f). For the R3F overlay, see [Batteries Included](/threejs-debug-view/guides/batteries-included/).
...
These exports are React-free and safe to use in tests, tools, or custom engines:
...
`threejs-debug-view/r3f` adds `useFrame` / `useThree` wiring, DOM overlays for labels and legends, and optional Leva controls. It calls the same headless runtime documented here — it does not duplicate the WebGPU pipeline.

...
The repo demo supports URL params such as `?debugView=emissive` and `?scene=overdraw`. For a fixed breakdown capture preset, open `/?capture=social&scene=overdraw` or run `pnpm capture:social` from the repository root.
